...
class CheckersStressTest extends Simulation {
  ...
  private val scn = scenario("Checkers Stress Test")
    //Erstellen
    .exec(
      http("create")
        .post("/game/create")
	...
	setUp(scn.inject(rampUsersPerSec(1).to(50).during(1.minutes))).protocols(httpProtocol)
}
